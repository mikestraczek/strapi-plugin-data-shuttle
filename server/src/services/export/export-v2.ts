...
import { CustomSlugToSlug, CustomSlugs } from '../../utils/constants';
...
function getPopulateFromSchema(slug, deepness = 5) {
  ...
  for (const [attributeName, attribute] of Object.entries(getModelPopulationAttributes(model))) {
    ...
    if (isComponentAttribute(typedAttribute)) {
      populate[attributeName] = getPopulateFromSchema(typedAttribute.component, deepness - 1);
    } else if (isDynamicZoneAttribute(typedAttribute)) {
...
function buildSlugHierarchy(slug, deepness = 5) {
  ...
  for (const [attributeName, attribute] of Object.entries(getModelPopulationAttributes(model))) {
    ...
    if (isComponentAttribute(typedAttribute)) {
      hierarchy[attributeName] = buildSlugHierarchy(typedAttribute.component, deepness - 1);
    } else if (isDynamicZoneAttribute(typedAttribute)) {
      hierarchy[attributeName] = Object.fromEntries(
        typedAttribute.components.map((componentSlug) => [
          componentSlug,
          buildSlugHierarchy(componentSlug, deepness - 1),
        ])
      );
    } else if (isRelationAttribute(typedAttribute)) {
      const relationHierarchy = buildSlugHierarchy(typedAttribute.target, deepness - 1);
